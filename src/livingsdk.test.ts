...
describe('LivingSDK: ', () => {
	...
	describe('._insert()', () => {
		...
		it('insert an ID to StorageApp', () => {
			return createMaxLSDK().get(lsd.appId, lsdktemplates.admin)
				...
				.then((storage: any) => {
					return storage.app.sdkinsert({id: `[JS]${(new Date()).toDateString()}`});
				});
		}).timeout( 10000 );
		...
		it('auto relog login', () => {
			...
			return lsdk.get(lsd.appId, lsdktemplates.admin)
				...
				.then((storage: any) => {
					return storage.app.sdkinsert({id: '[JS] before relogged'})
						.then(() => {
							return storage;
						})
				...
				.then((storage) => {
					return storage.app.sdkinsert({id: '[JS] relogged'})
				});
		}).timeout( 10000 );
	...
	describe('._update()', () => {
		...
		// 	.then((records: any) => {
		// 		for (let i of records) {
		// 			return i.sdkupdate({id: '[JS] updated'}).catch((err: AxiosError) => {
		// 				if (err.response && err.response.status === 403) {
		// 					(<any>lsdk).session = lsdk.login();
		// 					return i;
		// 				}
		...
		// 	})
		// 	.then((record: any) => {
		// 		return record.sdkupdate({id: '[JS] updated after relog'});
		// 	});
		// }).timeout(5000);
